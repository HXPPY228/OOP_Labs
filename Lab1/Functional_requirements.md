...
## 3. Стирание фигур
#### Описание: 
Пользователь может стереть фигуру, указав координаты (X, Y), где находится начальная точка фигуры.
#### Детали:
Если в точке (X, Y) найдена фигура (сравниваются координаты начала фигуры), она полностью удаляется с холста.
Поддерживаются все типы фигур (прямоугольник, круг, треугольник) с учетом их размеров и форм.
#### Реализация: 
Метод Erase ищет фигуру в списке _shapes по координатам. Для каждой фигуры выполняется очистка области с учетом её параметров, после чего фигура удаляется из списка.
## 4. Перемещение фигур
#### Описание: 
...
#### Детали:
### Сохранение: 
Холст записывается в текстовый файл построчно, где каждая строка соответствует строке массива _canvasArray.
### Загрузка: 
Данные из файла считываются в массив _canvasArray, а фоновый символ определяется как наиболее часто встречающийся символ, не являющийся символом фигуры.
#### Реализация:
Метод Save использует StreamWriter для записи массива в файл.
Метод Load считывает файл с помощью File.ReadAllLines и вызывает FindBackgroundColor для определения фона.
## 8. Очистка холста
#### Описание: 
